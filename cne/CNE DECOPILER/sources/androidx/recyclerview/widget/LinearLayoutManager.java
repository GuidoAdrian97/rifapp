...
public class LinearLayoutManager extends RecyclerView.LayoutManager implements ItemTouchHelper.ViewDropHandler, RecyclerView.SmoothScroller.ScrollVectorProvider {
        ...
        if (state.willRunPredictiveAnimations() && getChildCount() != 0 && !state.isPreLayout() && supportsPredictiveItemAnimations()) {
            ...
            for (int i5 = 0; i5 < size; i5++) {
                ...
                if (!viewHolder.isRemoved()) {
                    char c = 1;
                    if ((viewHolder.getLayoutPosition() < position ? true : DEBUG) != this.mShouldReverseLayout) {
                        c = 65535;
                    }
                    if (c == 65535) {
            ...
            if (i3 > 0) {
                ...
                fill(recycler2, this.mLayoutState, state2, DEBUG);
            }
            if (i4 > 0) {
    ...
    private boolean updateAnchorFromChildren(RecyclerView.Recycler recycler, RecyclerView.State state, AnchorInfo anchorInfo) {
        ...
        if (focusedChild == null || !anchorInfo.isViewValidAsAnchor(focusedChild, state)) {
            ...
            if (!state.isPreLayout() && supportsPredictiveItemAnimations()) {
                ...
                boolean z4 = (decoratedEnd > startAfterPadding || decoratedStart >= startAfterPadding) ? DEBUG : true;
    ...
    private boolean updateAnchorFromPendingData(RecyclerView.State state, AnchorInfo anchorInfo) {
        ...
        boolean z = DEBUG;
        if (!isPreLayout && (i = this.mPendingScrollPosition) != -1) {
        ...
        while (i2 != i3) {
            ...
            if (position >= 0 && position < itemCount) {
                if (!((RecyclerView.LayoutParams) childAt.getLayoutParams()).isItemRemoved()) {
                    boolean z3 = (decoratedEnd > startAfterPadding || decoratedStart >= startAfterPadding) ? DEBUG : true;
                    boolean z4 = (decoratedStart < endAfterPadding || decoratedEnd <= endAfterPadding) ? DEBUG : true;
                    if (!z3 && !z4) {
                        return childAt;
                    }
                    if (z) {
